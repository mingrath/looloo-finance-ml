...
- **Demonstrated feed — SIP (revised per [#14](https://github.com/mingrath/looloo-finance-ml/issues/14)):** the frozen contract now pins `feed=sip` (full-market consolidated SIP tape), revised from `feed=iex` to close the ~68% coverage gap surfaced in [#12](https://github.com/mingrath/looloo-finance-ml/issues/12). **This does not change the outcome.** §30's blanket prohibition on reproducing/distributing Alpaca-served market data controls any feed (IEX or SIP) accessed *via* the Alpaca API, so SIP-derived output stays non-redistributable on the same footing as the IEX feed; and because the consolidated SIP tape is the NASDAQ OMX / NYSE market data whose subscriber agreements §30 incorporates by reference (line 19; point 3 below), those incorporated terms reinforce the restriction. The IEX-specific Derived-Data asymmetry (the allowlist would qualify only if sourced *directly* from IEX — see below / escalation route 2) never applied to the Alpaca-served feed and does not apply to SIP either. The publish posture (this review's outcome + [#9](https://github.com/mingrath/looloo-finance-ml/issues/9)(a)) is therefore **UNCHANGED** — real SIP-derived aggregates stay local, access-controlled, and git-ignored under the Retention & purge policy below.
...
2. **Re-source daily bars directly from IEX** as a Data Subscriber and publish under IEX's Derived Data policy with required attribution. Since [#14](https://github.com/mingrath/looloo-finance-ml/issues/14) the demonstrated feed is **SIP, not IEX**, so this route now re-sources to a *different* feed than the one demonstrated — a real escalation here would swap the demonstrated SIP feed for a directly-subscribed IEX feed to obtain the Derived Data publish right. It stays documented and reversible.
