...
- **Execution:** signal at the scheduled close timestamp of the last valid XNYS session in the week; inspect only decision/entry bars; use the `label_fill_stream` adjusted SIP consolidated daily-bar open proxy for the scheduled next valid session timestamp unless the calendar-overlap rule skips the rebalance; use the same stream's adjusted SIP consolidated daily-bar close proxy for the fifth-session exit, charge the same one-way cost, set positions to zero, then schedule the next eligible weekly target. The XNYS calendar supplies ordering timestamps, not official auction prices; SIP consolidated OHLC are research proxies for auction prices.
...
- P&L-like diagnostics use the `label_fill_stream` adjusted SIP consolidated daily-bar open/close proxies with explicit 1 bp fee + 5 bp slippage baseline and 0/3/6/12 bp sensitivity. The XNYS schedule supplies timestamps and ordering, not auction prices; no cost-free result is reported as evidence and no return is promised.
...
- why the scheduled XNYS weekly-close signal uses a logical close timestamp, the next valid session's SIP consolidated daily-bar open proxy, and the calendar's actual early-close timestamp;
...
- The fixed basket has survivorship bias; the five-year sample may not generalize; the adjusted SIP consolidated fill/mark proxy is not an official XNYS auction quote or literal executable quote; no live feed, broker, shorting, borrow, financing, tax, or real-money behavior is tested; and no result guarantees future returns.
